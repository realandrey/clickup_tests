...
def test_create_task(create_task_fixture):
    ...
    with allure.step("Проверка, что задача была успешно создана"):
        assert status_code == 200, f"Ожидал статус 200, получил {status_code}"
        assert task_id, "ID задачи пустой"
        assert (
            task_data.get("name") == "Test Task"
        ), f"Ожидал имя 'Test Task', получил: {task_data.get('name')}"
        allure.attach(
            str(task_id),
            name="Created Task ID",
...
def test_get_task(task_api, create_task_fixture):
    ...
    with allure.step("Получение задачи через task_api.get_task"):
        ...
        ), f"Ожидал 200, получил {response.status_code}"
        data = response.json()
...
def test_delete_task(task_api, create_task_fixture):
    ...
    with allure.step("Удаление задачи через task_api.delete_task"):
        ...
        ), f"Ожидал 204, получил {delete_response.status_code}"
    ...
    with allure.step("Проверка что задача удалена через task_api.get_task"):
        ...
        ), f"Ожидал 404, получил {response_get.status_code}"
...
@pytest.mark.parametrize(
    "payload, expected_status", [({}, 400), ({"name": ""}, 400), ({"name": None}, 400)]
)
def test_create_task_negative(task_api, get_list_fixture, payload, expected_status):
    list_id = get_list_fixture["id"]

    with allure.step(
        "Попытка создать задачу с невалидными данными через task_api.create_task"
    ):
        response = task_api.create_task(list_id, payload)
        assert (
            response.status_code == expected_status
        ), f"Ожидал {expected_status}, получил {response.status_code}"
...
def test_get_task_negative(task_api, fake_task_id):
    with allure.step("Попытка получить фейковую задачу через task_api.get_task"):
        response = task_api.get_task(fake_task_id)
        assert (
            response.status_code == 401
        ), f"Ожидал 401, получил {response.status_code}"
